...
class DeepWalk(torch.nn.Module):
    """The DeepWalk model from DeepWalk: Online Learning of Social Representations
    ...
    Args:
        ...
        weight_key (str): The name of the DGL graph edata that contains the edge weights
    """

    def __init__(self, graph, embedding_dim, walk_length, context_size, walks_per_node, num_negative_samples, weight_key):
        super().__init__()
    ...
    def pos_sample(self, batch):
        ...
        random_walks, _ = random_walk(
            self.graph, batch, length=self.walk_length, prob=self.weight_key)
